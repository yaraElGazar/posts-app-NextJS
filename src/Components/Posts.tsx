...
const Posts = (props: {posts: Post[], type: string}) => {
    ...
    const toggleLike = (post: Post) => {
        ...
        if(post.liked) {
            ...
            localStorage.setItem("liked", JSON.stringify(...[liked.filter((like) => like.id !== post.id )]));
        }
    ...
    return (
        <>
        {props.posts.map((post: Post) => <Card key={post.id} post={post} handleLike={toggleLike} type={props.type} /> )}
        </>
    );
};
